...
def convert_to_shuangpin(pinyin):
    ...
    if pinyin.startswith('zh'):
        ...
        # その他のzhで始まる場合
            ...
            if final:
                final_code = FINAL_MAP.get(final, final[0] if final else '')
                return 'z' + final_code
            else:
                return 'z'
...
def main():
    ...
    """
    if len(sys.argv) < 2:
        print("Usage: python convert_to_shuangpin.py <input_file> [output_file]")
        sys.exit(1)
    
    input_file = sys.argv[1]
    output_file = sys.argv[2] if len(sys.argv) > 2 else input_file + '.shuangpin'
    ...
    try:
        ...
        print(f"変換された行数: {len(converted_lines)}")
