...
# -*- coding: utf-8 -*-
"""
删除主词库中包含不在单字列表中的单字的词

逻辑：以单字列表为准，删除主词库中包含不在单字列表中的单字的所有词

用法:
    python3 remove_words_with_missing_chars.py <single_char_file> <main_dict_file>
"""
...
def clean_dict_file(single_char_file, main_dict_file):
    """清理主词库文件，删除包含不在单字列表中的单字的词"""
    ...
    # 创建备份
    backup_file = main_dict_file + '.backup.' + datetime.now().strftime('%Y%m%d_%H%M%S')
    print(f'创建备份文件: {backup_file}')
    shutil.copy2(main_dict_file, backup_file)
    ...
    # 写入新文件
    print('正在写入清理后的文件...')
    with open(main_dict_file, 'w', encoding='utf-8') as f:
    ...
    if removed_words_count:
        ...
        for missing_char in sorted(removed_words_count.keys()):
            ...
            if len(words) <= 5:
                ...
                print(f'    示例: {", ".join(words[:5])} ... (还有 {len(words) - 5} 个)')
...
def main():
    ...
    success = clean_dict_file(single_char_file, main_dict_file)
