...
| `:IMExport` | 导出词库 | 强制用 DB 覆盖 YAML |
| `:IMInfo` | 显示词库信息 | 显示词库详细信息（路径、大小、条目数等） |
...
| 命令 | 功能 | 说明 |
|------|------|------|
| `:IMAdd` | 批量添加词 | 打开批量添加界面，可输入多行 `编码 词` |
| `:IMRemove` | 删除词 | 支持批量删除多个词 |
| `:IMReorder` | 重排序 | 调整词在对应编码下的显示顺序 |
| `:IMSearch` | 搜索词 | 在词库中搜索指定词，显示所有编码 |

#### 插件管理命令

| 命令 | 功能 | 说明 |
|------|------|------|
| `:IMReload` | 重新加载插件 | 重新加载整个插件和词库 |

### 详细说明

#### 词库管理命令

##### `:IMImport` - 导入词库（完全以 YAML 为准）

**功能**：将 YAML 文件完整导入到数据库，完全以 YAML 为准。

**逻辑**：
- 清空数据库中的所有数据
- 从 YAML 文件完整导入所有数据
- 确保数据库和 YAML 文件内容完全一致
- **如果 YAML 中删除了某些词，数据库中的这些词也会被删除**

**使用场景**：
- 首次导入词库
- 需要完全重置数据库，使其与 YAML 文件保持一致
- YAML 文件有重大修改，需要完全同步
...
**功能**：将 YAML 文件中的新数据同步到数据库，只增加不删除。

**逻辑**：
- **只增加**：只插入 YAML 中有但数据库中没有的新数据
- **不删除**：如果 YAML 中删除了某些词，数据库中的这些词**不会被删除**（保留在数据库中）
- 增量同步，不会影响数据库中已存在的数据

**使用场景**：
- 在 YAML 文件中手动添加了新编码和词条
- 需要将新添加的内容同步到数据库，以便输入法使用
- 日常维护词库时，通过编辑 YAML 文件来添加新词
...
**功能**：将数据库中的数据导出到 YAML 文件，强制用数据库覆盖 YAML。

**逻辑**：
- 读取数据库中的所有数据
- **强制覆盖** YAML 文件
- YAML 文件的内容会被完全替换为数据库中的内容
- **如果数据库中有但 YAML 中没有的词，会被添加到 YAML**
- **如果 YAML 中有但数据库中没有的词，会被删除**

**使用场景**：
- 数据库中的数据是最新的，需要更新 YAML 文件
- 在输入法使用过程中自动添加的词需要同步回 YAML
- 需要以数据库为准，统一数据源
...
**功能**：显示当前 ZFVimIM 插件的完整状态信息，帮助你快速诊断和了解词库加载情况。
...
| 功能 | 命令 | 数据源 | 目标 | 逻辑 | 删除行为 |
|------|------|--------|------|------|----------|
| **导入** | `:IMImport` | YAML | DB | 完全以 YAML 为准，清空 DB 后重新导入 | YAML 中删除的，DB 中也会删除 |
| **同步** | `:IMSync` | YAML | DB | 只增加 YAML 里有但 DB 没有的 | YAML 中删除的，DB 中**不删除** |
| **导出** | `:IMExport` | DB | YAML | 强制用 DB 覆盖 YAML | DB 中没有的，YAML 中会被删除 |

**使用建议**：
- **日常添加新词**：编辑 YAML → `:IMSync`（推荐）
- **批量添加新词**：使用 `:IMAdd` 批量添加 → 自动保存到 YAML → 自动同步到 DB
- **完全重置**：修改 YAML → `:IMImport`
- **以数据库为准**：使用输入法自动添加的词 → `:IMExport` 同步回 YAML

### 已废弃的命令（保留兼容性）

以下命令已废弃，建议使用新的 `IM*` 命令：

- `:ZFVimIMReload` → 使用 `:IMReload`
- `:ZFVimIMClear` → 使用 `:IMClear`
- `:ZFVimIMInfo` → 使用 `:IMInfo`
- `:ZFVimIMSync` → 使用 `:IMSync`
- `:ZFVimIMExport` → 使用 `:IMExport`
- `:ZFVimIMImport` → 使用 `:IMImport`

### 词库管理命令

#### `:ZFVimIMImport` - 导入词库（完全以 YAML 为准）
...
**说明**：
- 会清空并重新创建数据库
- 导入过程可能需要一些时间（取决于 YAML 文件大小）
- 导入完成后自动重新加载词库

#### `:ZFVimIMSync` - 同步词库（只增加，不删除）
...
**说明**：
- 只插入新数据，不会删除数据库中的现有数据
- **如果 YAML 中删除了某些词，这些词仍会保留在数据库中**（不删除）
- 同步速度快，适合频繁使用
- 同步完成后自动重新加载词库

**示例输出**：
```
ZFVimIM: Syncing YAML to database...
  YAML: /Users/tetsuya/.local/share/nvim/lazy/ZFVimIM/dict/sbzr.userdb.yaml
  DB:  /Users/tetsuya/.local/share/nvim/lazy/ZFVimIM/dict/sbzr.userdb.db
同步词库: .../sbzr.userdb.yaml -> .../sbzr.userdb.db
============================================================
读取数据库中已存在的数据...
  数据库中已有 778127 条记录
...
#### `:ZFVimIMExport` - 导出词库（强制用 DB 覆盖 YAML）
...
**说明**：
- 会完全覆盖 YAML 文件，原有内容会被替换
- 建议在导出前备份 YAML 文件
- 导出完成后，YAML 文件与数据库完全一致

### 三个功能的对比

| 功能 | 命令 | 数据源 | 目标 | 逻辑 | 删除行为 |
|------|------|--------|------|------|----------|
| **导入** | `:ZFVimIMImport` | YAML | DB | 完全以 YAML 为准，清空 DB 后重新导入 | YAML 中删除的，DB 中也会删除 |
| **同步** | `:ZFVimIMSync` | YAML | DB | 只增加 YAML 里有但 DB 没有的 | YAML 中删除的，DB 中**不删除** |
| **导出** | `:ZFVimIMExport` | DB | YAML | 强制用 DB 覆盖 YAML | DB 中没有的，YAML 中会被删除 |

**使用建议**：
- **日常添加新词**：编辑 YAML → `:ZFVimIMSync`（推荐）
- **批量添加新词**：使用 `:IMAdd` 批量添加 → 自动保存到 YAML → 自动同步到 DB
- **完全重置**：修改 YAML → `:ZFVimIMImport`
- **以数据库为准**：使用输入法自动添加的词 → `:ZFVimIMExport` 同步回 YAML
